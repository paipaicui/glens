    ...
    <van-popup v-model="isShowSearch" position="right" duration=".1" :overlay="false" :style="{ width: '100%' }">
      <div style="height: 100vh">
        ...
        <div class="search-list" ref="searchList">
          <van-loading v-if="searchLoading==true" size="24px" vertical style="padding-top:2rem">加载中...</van-loading>
...
export default {
  ...
  methods: {
    ...
    showSearchBox(done, title, tips) {
      ...
      setTimeout(() => {
        this.$refs.searchList.addEventListener('scroll', this.pulldown);
      }, 0);
    },
    //搜索框
    filter(val) {
    ...
    pulldown() {
      ...
      let scrollHeight = this.$refs.searchList.scrollHeight;
      let clientHeight = this.$refs.searchList.clientHeight;
      if (
        ...
        _self.list().then((res) => {
          if (res) {
            if (res.data.length > 0) {
              _self.searchList = [..._self.searchList, ...res.data]; //拼接数组
              _self.searchList1 = [..._self.searchList, ...res.data];
              _self.dropUp = false;
            } else {
              res.isOver = true;
            }
